...
- **NEVER extract artifacts from the final Docker stage**: The CentOS verification stage (stage 4) runs `./opencode` as `testuser`, which patchelf's the binary with `/home/testuser/.opencode/lib/...` hardcoded. Extracting from this stage ships a binary that only works for that specific user/path. **Always use `./docker/build.sh`** which extracts from the `assembler` stage (clean, un-patched binary). Never manually run `docker create opencode-builder` + `docker cp`.
